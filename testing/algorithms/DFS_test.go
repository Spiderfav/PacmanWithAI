...
func TestDFSSearch(t *testing.T) {
	...
	path := algorithms.DFSearch(gameGridDFS, 20, 20, 256, 256, 20)
...
func TestDFSSearchStartEnd(t *testing.T) {
	var gameGridDFS [][]mazegrid.MazeSquare = algorithms.DFS(256, nil, 20)
	path := algorithms.DFSearch(gameGridDFS, 20, 20, 5120, 5120, 20)
...
func BenchmarkDFSSearch(b *testing.B) {
	for i := 0; i < b.N; i++ {
		...
		algorithms.DFSearch(gameGridDFS, 20, 20, 20*mazeSizeOriginal, 20*mazeSizeOriginal, 20)
	}
}
